...
export async function launchBrowser(options) {
  ...
  /**
   * A launcher that exits is *not* by itself a startup failure.
   *
   * On Windows this Chrome hands the session to another process: the spawned launcher
   * exits — normally with code 0 — while the real browser is still starting and has
   * not bound the debugging port yet. Measured on the POC machine, that window ranges
   * from under a second to well over five, and the *same* flag set succeeds or fails
   * depending only on how loaded the machine is. So a refused connection alongside a
   * dead launcher is evidence that nothing is listening yet, not proof that nothing
   * ever will.
   *
   * The exit code is what separates the two cases, and it is available for free:
   *
   * - **Non-zero**: the executable ran and failed. Nothing is coming, so report it
   *   immediately instead of waiting out the readiness deadline — this is the case
   *   that used to hide a refusal behind a generic 45s timeout.
   * - **Zero**: a handoff. Keep waiting for the full readiness deadline. Treating this
   *   as fatal is exactly the bug this branch exists to fix (it broke `npm run poc`
   *   entirely on Windows), so it must never be reintroduced by a short grace period.
   */
