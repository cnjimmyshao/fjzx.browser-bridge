...
本报告采用 `d734d9f` 上这一次完整运行（780.5s）作为证据。同一天还完整跑过 A–G 三次（`5ab9579` 一次、`11ebd13` 两次），其中 `11ebd13` 的第一次用于排查 harness 自身缺陷；`d734d9f` 上另有两次完整运行，第一次暴露了 B 场景断言的 off-by-one（见下），第二次即本报告采用的这一次。基线回收时间稳定在 30.8–30.9s，停止保活后的回收时间同为 30.8–31.0s，Service 恢复后的重连耗时 11.1–13.2s，保活窗口内回收每次都是 0 次。
