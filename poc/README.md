...
>
> **不要与本仓库的 `tests/poc/` 混淆**：那个是 **V1 扩展的端到端 POC**（`npm run poc`，跑 `src/` 里的真扩展 + 一个 mock Service）；本目录是 **issue #13 的调研 POC**，跑的是一个额外声明了 `cookies` / `scripting` 的实验扩展，只用于验证浏览器能力边界。两者都刻意保持零第三方依赖，是否合并成一份 harness 留给评审决定。
